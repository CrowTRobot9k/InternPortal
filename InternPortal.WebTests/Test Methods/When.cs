...
namespace InternPortal.WebTests.Test_Methods
{
    public partial class TestMethods : BaseWebTest
    {
        ...
        [When(@"text entered (.*) (.*)")]
        [When(@"questionaire text entered (.*) (.*)")]
        public void WhenTextEntered(string inputId, string text)
        {
            ...
            {
                var random = ExtensionMethods.RandomString(8);
                input.SendKeys(random);
            }
            else if (text.ToLower() == "randomemail")
            {
                var email = ExtensionMethods.RandomString(8) + "@gmail.com";
        ...
        [When(@"saved text entered (.*) (.*) (.*)")]
        public void WhenSavedTextEntered(string inputId, string text, bool useSaved)
        {
            WaitElementIdExist(inputId);
            ...
            {
                var random = ExtensionMethods.RandomString(8);
                if (useSaved && ExtensionMethods.SavedString != null)
                {
                    input.SendKeys(ExtensionMethods.SavedString);
                }
                else
                {
                    ExtensionMethods.SavedString = random;
                    input.SendKeys(ExtensionMethods.SavedString);
                }
            }
            else if (text.ToLower() == "randomemail")
            {
                ...
                {
                    ExtensionMethods.SavedString = email;
                    input.SendKeys(ExtensionMethods.SavedString);
                }
            }
            else
        ...
        [When(@"kendo dropdown with id (.*) value (.*)")]
        public void WhenKendoDropDownPicked(string dropdownId, string value)
        {
            Thread.Sleep(500);
        ...
        public void WhenCheckBoxIsChecked(string checkBoxId)
        {
            var wait = new WebDriverWait(Driver, TimeSpan.FromSeconds(PageTimeOut));
            wait.Until(ExpectedConditions.ElementExists(By.Id(checkBoxId)));
        ...
        public void WhenRadioIsSelected(string radioId, string value)
        {
            var wait = new WebDriverWait(Driver, TimeSpan.FromSeconds(PageTimeOut));
            wait.Until(ExpectedConditions.ElementExists(By.XPath("//input[@id='" + radioId + "' and @value='" + value + "']")));

            IWebElement radio = Driver.FindElement(By.XPath("//input[@id='" + radioId + "' and @value='" + value + "']"));
            radio.Click();
            TakeScreenShot();
        }

        [When(@"Pause (.*)")]
        public void WhenPause(int milliseconds)
        {
            System.Threading.Thread.Sleep(milliseconds);
        }

        [When(@"button is clicked Wait time(.*) (.*) (.*)")]
        public void WhenButtonIsClickedWaitTime(string parentClassName, string tagName, int waitTime)
        {
            var wait = new WebDriverWait(Driver, TimeSpan.FromSeconds(waitTime));
            IWebElement divclass = wait.Until(ExpectedConditions.ElementExists(By.ClassName(parentClassName)));
            IWebElement closeBtn = divclass.FindElement(By.TagName(tagName));
            closeBtn.Click();
        }

        [When(@"button (.*) is clicked")]
        public void WhenButtonClicked(string buttonId)
        {
            var wait = new WebDriverWait(Driver, TimeSpan.FromSeconds(PageTimeOut));
            wait.Until(ExpectedConditions.ElementExists(By.Id(buttonId)));
        ...
        public void WhenDropDownPicked(string dropDownId, string value)
        {
            ...
            var dropdown = Driver.FindElement(By.Id(dropDownId));
